        ...
        {dbStructure && (
            <div className="mt-8 w-full overflow-x-auto">
              {Object.keys(dbStructure).map((dbName) => (
                <div key={dbName} className="mb-8 p-4 border border-gray-300">
                  <h2 className="text-2xl font-bold text-white">{dbName}</h2>
                  {Object.keys(dbStructure[dbName as keyof AdCampaignData] as Record<string, any>).map((collectionName) => (
                    <div key={collectionName} className="mt-4">
                      <h3 className="text-xl font-semibold text-white">
                        Collection: {collectionName}
                      </h3>
                      {typeof (dbStructure[dbName as keyof AdCampaignData] as Record<string, any>)[collectionName] === "string" ? (
                        <p className="text-gray-300">{(dbStructure[dbName as keyof AdCampaignData] as Record<string, any>)[collectionName]}</p>
                      ) : (
                        <table className="min-w-full bg-white text-black border-collapse">
                          <thead>
                            <tr>
                              {Object.keys((dbStructure[dbName as keyof AdCampaignData] as Record<string, any>)[collectionName][0] ?? {}).map(
                                (key) => (
                          ...
                          <tbody>
                            {(dbStructure[dbName as keyof AdCampaignData] as Record<string, any>)[collectionName].map((doc: any, index: number) => (
                              <tr key={index}>
